        ...
        try {
            ...
            if (allEvents) {
                ...
                for (const event of allEvents.data) {
                    ...
                    let matchRecorded = await matchesServices.findOneRecord(condition);

                    if (!matchRecorded) {
                        matchRecorded = await matchesServices.createRecord(values);
                    } else {
                    ...
                    for (const goalscorers of event.goalscorers) {
                        ...
                        const condition = {
                            where: { event_key: event.event_key, time: goalscorers.time, home_scorer: goalscorers.home_scorer, away_scorer: goalscorers.away_scorer }
                        };

                        const values = {
                            match_id: matchRecorded.dataValues.id,
                        ...
                        const goalscorerRecorded = await goalscorersServices.findOneRecord(condition);

                        if (!goalscorerRecorded) {
                    ...
                    for (const statistics of event.statistics) {
                        ...
                        const statisticsRecorded = await statisticsServices.findOneRecord(condition);
